...
var pm_li=pm_box.getElementsByTagName("tr")[0];//单个用户
var btn_t=document.querySelector(".btn1");//下滑
var btn_j=document.querySelector('.btn2');//起跳
...
var back=document.querySelector(".ending_b_back");//返回主页
var share=document.querySelector(".ending_b_share");//返回主页
var p_name=document.querySelector(".p_name");//玩家名字
var p_btn=document.querySelector(".p_btn");//确定按钮
...
btn2.src="img/pk_icon.png";
star.src="img/pk_start.png";

//window.location.href="https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx225b4215ece069fc&redirect_uri=https%3A%2F%2Fttpb.applinzi.com%2Fcode.php&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect";
...
var p_bol=true;//只能点一次确定
//console.log(localStorage.key(0),localStorage.getItem(localStorage.key(0)));
p_btn.addEventListener("click",function(){
	var time=new Date().getMonth()+1+"-"+new Date().getDate()+" "+new Date().getHours()+":"+new Date().getMinutes();
	var id=new Date().getTime();
	if(p_bol){
		if(p_name.value.length>8){
			alert("你的东西过长，请重新输入！");
			return
		}else if(p_name.value.length==""){
			alert("请输入您的大名！");
			return
		}
		localStorage.setItem(id+"info",p_name.value+"/"+time+"/"+zongfen.value);
		p_name.value="";
		p_name.placeholder="您已输入！"
		p_bol=false;
	}
})
var lol=[],lpl=[],skt=[],rng=[];//lol存分数,lpl存key,skt存下标,rng是二维数组;
function phb(index){
	for (var t=0;t<localStorage.length;t++) {
		var Lkey=localStorage.key(t);
		var Lscore=localStorage.getItem(Lkey).split("/")[2];
		rng.push(localStorage.getItem(Lkey).split("/"));
		lol.push(Number(Lscore));
		lpl.push(localStorage.getItem(Lkey));
	}
	lol.sort(function(a,b){return b-a});
	...
	if(skt.length>=10){
		for (var g=10;g<skt.length;g++) {
			localStorage.removeItem(localStorage.key(skt[g]));
		}
		for (var s=0;s<10;s++) {
			var info=pm_li.cloneNode(true);
			info.querySelectorAll("th")[0].innerHTML=rng[skt[s]][0];
			info.querySelectorAll("th")[1].innerHTML=rng[skt[s]][1];
			info.querySelectorAll("th")[2].innerHTML=rng[skt[s]][2];
			pm_box.appendChild(info);
		}
	}else{
		for (var s=0;s<skt.length;s++) {
			var info=pm_li.cloneNode(true);
			info.querySelectorAll("th")[0].innerHTML=rng[skt[s]][0];
			info.querySelectorAll("th")[1].innerHTML=rng[skt[s]][1];
			info.querySelectorAll("th")[2].innerHTML=rng[skt[s]][2];
...
//点击排行榜
paiming.addEventListener("click",function(){
	stop_cont.style.display="none";
	game.style.display="none";
	ready.style.display="block";
	kobe=0;
...
//点中 继续游戏
btn_jixu.addEventListener("click",function(){
...
//点击冲刺
power_num.addEventListener("click",function(){
	if(chongci_bol1){
		chong_num++;
		chongci_bol1=false;
		chongci_bol=false;
...
function allBg(){
	...
	ctx.drawImage(bg,-bg_index,0,dW,dH*1.16);
	ctx.drawImage(bg,dW-bg_index,0,dW,dH*1.16);
}
allBg()
...
//	console.log(e.keyCode);
	if(e.keyCode==32){
		if(chongci_bol1){
			...
			chongci_bol=false;
			hero.jindex=0;
			hero.jindex2=0;//第一跳计数归零，不归零冲刺完再向上跳会少了一部分动作
			hero.jindex3=0;//第二跳计数归零
			hero.jindex4=0;
		}
		e.preventDefault();
	}
